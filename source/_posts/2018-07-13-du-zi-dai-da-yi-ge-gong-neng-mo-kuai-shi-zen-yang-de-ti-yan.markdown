...
（三）：物理设计：根据ER图的关系，遵循**前三范式**的要求，将所有表设计出来。  
...
完成之后将所有表的关系图用MySQLWorkbench中的Reverse Engineer...画出来，将所有的需求串起来在脑海里模拟一遍，看有没有哪里进行增删改查的时候不太顺畅、表之间的关联关系太复杂的地方。  
...
4. 对修改与删除操作进行关联性测试，删除某资源引用的另一个资源或者该资源
...
这两个月其实还学到不少东西，毕竟是第一次自己设计一个比较大的功能，主要在设计与调研阶段中犯过不少问题，比如遗漏过很多关键点，导致开发的时候修改数据库结构与以前调研过的实现方式，这是因为以前在设计与调研的过程中没有经验与套路导致的，那么本文也主要是总结这些套路，让自己在以后有一套比较**系统**的解决问题的方式了。
