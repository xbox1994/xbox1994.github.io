...
当Java程序性能达不到既定目标，且其他优化手段都已经穷尽时，通常需要调整垃圾回收器来进一步提高性能，这就是JVM调优。

在调优的时候，我们必须要知道调的是什么，为什么要这样调，这些跟基础知识密不可分。

比如要知道内存结构、垃圾收集机制、算法、收集器，还要知道如何从GC日志中得到调试的效果，那么前几篇文章将带你回顾或者学习关于调优用到的JVM基础知识。在最后我会总结出一个调优的套路，不论在实战还是面试中都能用上
...
### 并行收集器
并行收集器Parallel关注**可控的吞吐量**，能精确地控制吞吐量与系统停顿是该收集器最大的特点，也是1.8的Server模式的默认收集器，使用多线程收集
...
CMS针对老年代，`标记-清除`算法。ParNew垃圾收集器是Serial收集器的多线程版本。

参数：`-XX:+UseConcMarkSweepGC`，默认开启`-XX:+UseParNewGC`

G1能独立管理整个GC堆（新生代和老年代），`标记-整理`算法，是1.9的Server模式的默认收集器
